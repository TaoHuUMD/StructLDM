...
from .StructNeRF_RenderPeople import VoxelHuman as StrNeRF_RENDERPEOPLE
...
class VoxelHumanGenerator(nn.Module):
    def __init__(self, model_opt, renderer_opt, blur_kernel=[1, 3, 3, 1], ema=False, full_pipeline=True, voxhuman_name=None, opt = None):
        super().__init__()
        ...
        #.
        #if df_opt is not None and df_opt.stage_1_fitting:
        #    self.build_embedding_stage1(df_opt)
        ...
        if model_opt.voxhuman_name == 'struct_nerf_renderpeople':
            VoxHuman_Class = StrNeRF_RENDERPEOPLE
        else:
            raise NotImplementedError
        
        reso = (model_opt.renderer_spatial_output_dim[0] // self.opt.nerf_ratio, model_opt.renderer_spatial_output_dim[1] // self.opt.nerf_ratio) #model_opt.renderer_spatial_output_dim
        self.renderer = VoxHuman_Class(opt, smpl_cfgs, out_im_res=tuple(reso), style_dim=self.style_dim)
    ...
    def forward(self, styles, cam_poses, focals, beta, theta, trans,
                ...
                w_space=False, gamma_list=None, beta_list=None, fix_viewdir=False, smpl_vts_wrd = None,
                        smpl_vts_can = None, batch = None, scale = 1):
        if w_space:
            latent = [None, None]
            raise NotImplementedError()
        else:
        ...
        if inv_Ks is None:
            if w_space:
                thumb_rgb, features, _sdf, mask, xyz, eikonal_term = \
                    self.renderer(cam_poses, focals, beta, theta, trans, styles=latent[0], \
                    return_eikonal=return_eikonal,
                    w_space=w_space, gamma_list=gamma_list, beta_list=beta_list,
                    fix_viewdir=fix_viewdir)
            else:
                thumb_rgb, features, _sdf, mask, xyz, eikonal_term = \
                    self.renderer(cam_poses, focals, beta, theta, trans, styles=latent, \
                    return_eikonal=return_eikonal, fix_viewdir=fix_viewdir, smpl_vts_wrd = smpl_vts_wrd,
                        smpl_vts_can = smpl_vts_can, batch = batch, scale = scale)
        else:
            thumb_rgb, features, _sdf, mask, xyz, eikonal_term = \
                self.renderer(cam_poses, focals, beta, theta, trans, styles=latent[0], \
