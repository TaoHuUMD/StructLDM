...
class DemoDataset(Dataset):
    def __init__(self, sample_data_path='./data/asset/sample_data.pkl'):
...
class DeepFashionDataset(Dataset):
    def __init__(self, path, transform, resolution=(256, 128), nerf_resolution=(128, 64), flist=None,
                 ...
                 random_flip=False,
                 exclude_list=None,
                 gaussian_weighted_sampler=False, sampler_std=0.3 / np.pi * 180, opt = None):
        self.path = path
    ...
    def __getitem__(self, index):
        try:
            ...
            # _img = _img.crop((128, 0, 512-128, 512))
...
class AISTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        meta = {
            'img': img, 
            'thumb_img': thumb_img,
            "trans": trans.float().clone(),
            'beta': beta.float().clone(),
            'theta': theta.float().clone(),
            'intrinsic': intrinsic.float().clone(), 
            'extrinsic': extrinsic.float().clone(),
            'is_fliped': need_flip,
            'image_name': self.img_fname_list[index],
        }
        return meta
