...
class FiLMSiren(nn.Module):
    ...
    def forward(self, input, style):
        ...
        #printy("FiLMSiren ", input.shape, style.shape)
        ...
        #printg(out.shape, gamma.shape, beta.shape)
...
class SirenGenerator(nn.Module):
    def __init__(self, D=5, W=128, style_dim=128, input_ch=3, input_ch_views=3, output_ch=4,
    ...
    def forward(self, x, styles):
        ...
        out_features = torch.cat([mlp_out, input_views], -1) if input_views is not None else mlp_out
        
        if self.input_ch_views:
          out_features = self.views_linears(out_features, styles)

        rgb = self.rgb_linear(out_features)

        outputs = torch.cat([sdf, rgb], -1)
        if self.output_features:
            outputs = torch.cat([outputs, out_features], -1)
...
# Basic nerf decoder
class NeRFDecoderNoView(nn.Module):
