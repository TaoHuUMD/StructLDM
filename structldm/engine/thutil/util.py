...
class Times_Cuda():
    ...
    def end(self):
        ...
        milliseconds = self.start_time.elapsed_time(self.end_time)/1000
        return milliseconds
        print('time for the second 20 iter',start_time.elapsed_time(end_time)/1000)  # milliseconds

def bbox2_torch(img):

    rows = torch.any(img, 1)
    cols = torch.any(img, 0)
    rmin, rmax = torch.where(rows)[0][[0, -1]]
    cmin, cmax = torch.where(cols)[0][[0, -1]]

    return rmin, rmax, cmin, cmax
    rmin, rmax, cmin, cmax = bbox2_torch(full_mask)
    full_mask_bbox = full_mask[rmin:rmax, cmin:cmax]


# if is_timing and iteration == 20:
#         start_time.record()

#     if is_timing and iteration == 40:
#         end_time.record()
#             # Waits for everything to finish running
#         torch.cuda.synchronize()
